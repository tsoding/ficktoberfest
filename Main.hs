...
pollEvents :: Github -> Owner -> Maybe ETag -> IO (Maybe ETag, Maybe Int, [Event])
...
pollLoop github owner lastEventId etag = do
  ...
  let unprocessedEvents =
        filter (\x -> lastEventId < Just (eventId x)) events
  mapM print unprocessedEvents
  maybe (return ()) (threadDelay . (* 1000000)) interval
  pollLoop
    github
    owner
    ((eventId <$> listToMaybe unprocessedEvents) <|> lastEventId)
    etag'
